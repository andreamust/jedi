...
class Distribution {
  ...
  insert(x) {
    if (Object.keys(this.data).includes(x)) this.data[x]++;
    else this.data[x] = 1;
  }
  mostFrequent() {
...
class Node {
  ...
  branch() {
    ...
    const subjects_complement = this.subjects.filter(
    ...
    for (const subject of subjects) {
      const properties = this.properties_per_subject[subject].filter(
        (p) => p !== this.most_used_property[0]
      );
    ...
    this.right = new Node(
      complement_properties_per_subject,
      complement_subjects_per_property
    );
  }
  print() {
    ...
    if (this.left && this.left.node_counter && this.most_used_property) {
      console.log(
        `${this.node_counter} -> ${this.left.node_counter} [label="${this.most_used_property[0]}"];`
      );
      this.left.print();
    }
    if (this.right && this.right.node_counter) {
      console.log(
        `${this.node_counter} -> ${this.right.node_counter} [label="complement"];`
      );
      this.right.print();
    }
  }
  serialize(file_name) {
    ...
    fs.writeFileSync(
      `./property_usage_hierarchy/${file_name}.json`,
      serialization
    );
  }
